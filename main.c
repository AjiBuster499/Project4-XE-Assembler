...
int main( int argc, char* argv[]){
	...
	while( fgets(line ,1024 , fp) != NULL  )
		...
		if( (line[0] >= 65) && ( line[0]<= 90 )) // this lets us know the line is between
			// A and capital Z.
			{
				newsym = strtok( line, " \t\n\r");
    ...
    while( fgets(line ,1024 , fp) != NULL  ) {
            ...
            // A and capital Z.
        {
            ...
            newdir = strtok(NULL, " \t\n\r");
            tok3 = strtok(NULL, "\t\n");
            if ((strcmp("RESB", newdir) == 0) || (strcmp("RESW", newdir) == 0) || (strcmp("START", newdir) == 0))   {
                linecount2++;
                continue;
            }
            if((strcmp("END", newdir) == 0)){
                tempsym = symbolReturn(symTab, tok3);
                if((tempsym == NULL) && (tok3 != NULL))
                {
                    printLine(lclone);
            ...
            newsym = strtok(line, " \t\n\r");
            newdir = strtok(NULL, " \t\n\r");
            if ((strcmp("RESB", newsym) == 0) || (strcmp("RESW", newsym) == 0) || (strcmp("START", newsym) == 0)) {
                linecount2++;
                continue;
            }
...
int addCtr(struct syminst tab[], char* symbol, char* tok3, unsigned int* ctr, char line[], int srcline)
{
    ...
    else if((strcmp("EXPORTS", symbol) == 0  ) || (strcmp("RESR", symbol) == 0))
    {
    ...
    else if((strcmp("WORD", symbol) == 0))
    {
        ...
        if((*ctr +(3) <= 0x8000) && ((temp <= 8388608 ) && ( temp >= -8388608)))
        {
            *ctr += 3;
            return 1;
        }
...
int validHex(char* string)
{
    ...
    {
        if((string[index] >= 65 && string[index] <= 70) || (string[index] >= 30 && string[index] <= 57)) //checks if this is a valid hex number
        {
            index++;
            continue;
        }
                   ...
                   char trec[][71], char mrec[][71], struct syminst inst[])
{
    ...
    if(strcmp(first, "BYTE") == 0)
    {
        ...
        else if(second[0] == 'C') //character, initiate character parsing procedure
        {
            ...
            for(int x = 1; x <= looplen; x++)
            {
                ...
                while((tempstring[ind] != '\0') && (counter <= 29))
                {
    ...
    if((strcmp("STCH", first) == 0) || (strcmp("LDCH", first) == 0))
    {
        sym = strtok(second, " ,");
